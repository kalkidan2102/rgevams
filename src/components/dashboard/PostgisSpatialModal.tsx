...
export const PostgisSpatialModal: React.FC<PostgisSpatialModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "radius" | "hazardZone" | "dbscan" | "geojson"
  >("radius");

  // Proximity query states
  const [selectedVolcanoId, setSelectedVolcanoId] =
    useState<string>("erta_ale");
  const [customLat, setCustomLat] = useState<number>(13.6);
  ...
  const handleRunProximity = async () => {
    ...
    try {
      const res = await fetch(
        `/api/spatial/earthquakes-radius?lat=${customLat}&lng=${customLng}&radiusKm=${radiusKm}`,
      );
      ...
      if (res.ok && data.earthquakes) {
        ...
        setProximityError(
          data.error || "Failed to execute spatial proximity search.",
        );
      }
    } catch (e: any) {
  ...
  const handleRunHazardZone = async () => {
    ...
    try {
      const res = await fetch(
        `/api/spatial/volcano-hazard-zone/${selectedVolcanoId}`,
      );
      const data = await res.json();
      if (res.ok && data.infrastructureAtRisk) {
        ...
        setHazardError(
          data.error ||
            "Failed to compute hazard zone infrastructure intersections.",
        );
      }
    } catch (e: any) {
  ...
  const handleRunDbscan = async () => {
    ...
    try {
      const res = await fetch(
        `/api/spatial/seismic-clusters?epsKm=${epsKm}&minPoints=${minPoints}`,
      );
      const data = await res.json();
  ...
  return (
    <div 
      className="fixed inset-0 z-9999 flex items-center justify-center p-4 sm:p-6 bg-slate-950/80 backdrop-blur-md animate-fade-in font-sans"
      onClick={onClose}
    >
      <div 
        className="bg-[#FAF9F5] dark:bg-slate-900 border-2 border-[#0E4A72]/20 dark:border-white/10 rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden text-slate-900 dark:text-white"
            ...
            <div className="space-y-4">
              <div className="bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900 p-4 rounded-2xl text-xs space-y-1">
                <div className="font-bold text-blue-900 dark:text-blue-200 flex items-center gap-2">
                  ...
                  <span>
                    WGS 84 Ellipsoidal Distance Calculation (`ST_DWithin` &amp;
                    `ST_Distance`)
                  </span>
                </div>
                <p className="text-blue-800 dark:text-blue-300 text-[11px] leading-relaxed">
                  PostGIS projects coordinates onto the WGS84 spheroid
                  (`EPSG:4326::geography`) to compute geodesically exact
                  earthquake distances in kilometers.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs">
                ...
                <div className="flex items-end gap-2">
                  ...
                  >
                    <RefreshCw
                      className={`w-3.5 h-3.5 ${proximityLoading ? "animate-spin" : ""}`}
                    />
                    <span>Run Query</span>
                  </button>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs font-mono text-slate-600 dark:text-slate-400">
                  <span>
                    Query Results:{" "}
                    <strong>{proximityResults?.length || 0}</strong>{" "}
                    earthquake(s) within {radiusKm} km
                  </span>
                  <span className="text-[10px] text-emerald-600 font-bold">
                    Spatial Index GiST Applied
                  </span>
                </div>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-900">
                      {proximityResults && proximityResults.length > 0 ? (
                        ...
                        <tr>
                          ...
                          >
                            No earthquake epicenters found within the {radiusKm}{" "}
                            km spatial buffer.
                          </td>
                        </tr>
                      )}
            ...
            <div className="space-y-4">
              <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900 p-4 rounded-2xl text-xs space-y-1">
                <div className="font-bold text-amber-900 dark:text-amber-200 flex items-center gap-2">
                  ...
                  <span>
                    Volcanic Buffer Intersection (`ST_Buffer` &amp; Spatial
                    Join)
                  </span>
                </div>
                <p className="text-amber-800 dark:text-amber-300 text-[11px] leading-relaxed">
                  Computes intersections between active volcanic vent exclusion
                  radii and critical national infrastructures (electrical grids,
                  geothermal fields, transport corridors).
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700">
                ...
                >
                  <RefreshCw
                    className={`w-3.5 h-3.5 ${hazardLoading ? "animate-spin" : ""}`}
                  />
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs font-mono text-slate-600 dark:text-slate-400">
                  <span>
                    ...
                    <strong>{hazardResults?.length || 0}</strong> facility(ies)
                    inside buffer
                  </span>
                </div>
                      ...
                      {hazardResults && hazardResults.length > 0 ? (
                        hazardResults.map((item, i) => (
                          ...
                          >
                            ...
                            <td className="p-3 font-mono font-bold text-amber-600">
                              {(Number(item.vulnerabilityIndex) * 100).toFixed(
                                0,
                              )}
                              % Risk
                            </td>
                        ...
                        <tr>
                          <td
                            ...
                            className="p-6 text-center text-slate-400 text-xs"
                          >
                            No critical facilities detected inside the
            ...
            <div className="space-y-4">
              <div className="bg-cyan-50 dark:bg-cyan-950/40 border border-cyan-200 dark:border-cyan-900 p-4 rounded-2xl text-xs space-y-1">
                <div className="font-bold text-cyan-900 dark:text-cyan-200 flex items-center gap-2">
                  ...
                  <span>
                    PostGIS Spatial Swarm Detection (`ST_ClusterDBSCAN`)
                  </span>
                </div>
                <p className="text-cyan-800 dark:text-cyan-300 text-[11px] leading-relaxed">
                  Identifies localized magma dyke intrusion swarms across the
                  Afar depression and Main Ethiopian Rift by clustering spatial
                  point coordinates within epsilon search distances.
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700">
                ...
                >
                  <RefreshCw
                    className={`w-3.5 h-3.5 ${dbscanLoading ? "animate-spin" : ""}`}
                  />
                    ...
                    {dbscanResults && dbscanResults.length > 0 ? (
                      dbscanResults.map((c, i) => (
                        ...
                        >
                          ...
                          <td className="p-3 font-mono text-slate-400 text-[10px]">
                            {Number(c.latitude).toFixed(2)}°N,{" "}
                            {Number(c.longitude).toFixed(2)}°E
                          </td>
                          <td className="p-3 font-mono text-slate-400 text-[10px]">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-xs text-slate-600 dark:text-slate-400 font-sans">
                  Direct GeoJSON export generated server-side using PostGIS
                  `ST_AsGeoJSON()`:
                </p>
                <button
                  onClick={() => {
