...
app.get("/api/mongodb/status", (req, res) => {
  res.json({
    ...
    collections: ["volcanoes", "earthquakes", "gnss_stations", "users", "audit_logs", "alerts"]
  });
});
...
const defaultVolcanoes = [
  {
    ...
    elevation: 613,
    coordinates: [13.60, 40.67], // [lat, lng]
    type: "Shield Volcano",
...
if (!fs.existsSync(USERS_FILE)) {
  ...
  try {
    ...
    defaultUsers.forEach(du => {
      if (!existing.some((u: any) => u.email.toLowerCase() === du.email.toLowerCase())) {
        existing.push(du);
        updated = true;
      }
    });
...
app.post("/api/auth/register", (req, res) => {
  ...
  if (!name || !email || !role || !password) {
    return res.status(400).json({ error: "Missing required registration fields (name, email/ID, role, password)" });
  }
  ...
  if (role !== "official" && role !== "admin" && role !== "superadmin") {
    return res.status(400).json({ error: "Invalid role selected for security clearance" });
  }
  ...
  const newUser = {
    ...
    institution: institution || (role === "admin" ? "ESSGI Directorate" : "Ethiopian Space Science and Geospatial Institute"),
    password,
    registeredAt: new Date().toISOString()
  };
...
app.post("/api/auth/login", (req, res) => {
  ...
  if (user.status === "rejected") {
    return res.status(403).json({
      error: "Access Denied: Your registration request was rejected by the Super Admin.",
      status: "rejected"
    });
  }
...
app.get("/api/admin/users", (req, res) => {
  ...
  if (userRole && userRole !== "superadmin" && userRole !== "admin" && userRole !== "official") {
    return res.status(403).json({ error: "Access denied. Only authorized personnel can view user accounts." });
  }
...
app.post("/api/admin/approve-user", (req, res) => {
  ...
  const performerName = req.headers["x-user-name"] || adminEmail || "Administrator";
  ...
  res.json({ success: true, message: `User ${targetEmail} authorized successfully.` });
});
...
app.post("/api/admin/users/:email/approve", (req, res) => {
  ...
  if (userRole !== "superadmin" && userRole !== "admin") {
    return res.status(403).json({ error: "Access denied. Super Admin privilege required to approve accounts." });
  }
  ...
  saveAuditLog({
    ...
    action: "approve_user",
    performedBy: String(performerName),
    performedByEmail: String(req.headers["x-user-email"] || "superadmin@essgi.gov.et"),
    performedByRole: String(userRole),
...
app.patch("/api/admin/users/:email/role", (req, res) => {
  ...
  const { role } = req.body;
  if (!role || (role !== "official" && role !== "admin" && role !== "superadmin" && role !== "guest")) {
...
app.delete("/api/admin/users/:email", (req, res) => {
  ...
  const targetEmail = req.params.email.toLowerCase();
  let users = getUsers();
  const userToDelete = users.find((u: any) => u.email.toLowerCase() === targetEmail);
...
app.post("/api/admin/users/seed-test-pending", (req, res) => {
  ...
  res.json({ success: true, message: "Demo pending registration created!", user: newUser });
});
...
app.post("/api/admin/alert-config", (req, res) => {
  ...
  const performerName = req.headers["x-user-name"] || req.body.adminName || "Administrator";
  const performerEmail = req.headers["x-user-email"] || req.body.adminEmail || "admin@essgi.gov.et";

  if (userRole && userRole !== "superadmin" && userRole !== "admin" && userRole !== "official") {
    return res.status(403).json({ error: "Access denied. Administrator clearance required to modify alert threshold parameters." });
  }
...
app.post("/api/admin/alert-config/test-dispatch", (req, res) => {
  ...
  const eventTitle = testEvent?.title || `M ${parseFloat(mag).toFixed(1)} Seismic Tremor near ${loc}`;
  ...
  const renderedSms = (config.smsTemplateText || "ESSGI URGENT ALERT: M{mag} Earthquake detected at depth {depth}km in {loc}.")
    .replace("{mag}", String(parseFloat(mag).toFixed(1)))
    .replace("{depth}", String(depth))
    .replace("{loc}", loc);

  const renderedEmailSubject = (config.emailSubjectTemplate || "[ESSGI GEODISASTER WARNING] M{mag} Seismic Tremor - {loc}")
    .replace("{mag}", String(parseFloat(mag).toFixed(1)))
    .replace("{depth}", String(depth))
    .replace("{loc}", loc);
...
function processEarthquakeAlert(eq: any, source: string = "Telemetry Ingest") {
  try {
    const config = getAlertConfig();
    const thresholdMag = typeof config.minMagnitude === "number" ? config.minMagnitude : 5.0;
    const thresholdDepth = typeof config.maxDepth === "number" ? config.maxDepth : 35;
    ...
    const dispatches = getAlertDispatches();
    const alreadyDispatched = dispatches.some((d: any) => d.eventId === eq.id || (d.location === eq.location && Math.abs(d.magnitude - eq.magnitude) < 0.1 && Math.abs(new Date(d.timestamp).getTime() - new Date(eq.dateTime).getTime()) < 60000));
    ...
    const renderedSms = (config.smsTemplateText || "ESSGI CRITICAL ALERT: M{mag} Earthquake detected at depth {depth}km in {loc}. Immediate advisory triggered.")
      .replace("{mag}", String(Number(eq.magnitude).toFixed(1)))
      .replace("{depth}", String(eq.depth || 10))
      .replace("{loc}", eq.location);

    const renderedEmailSubject = (config.emailSubjectTemplate || "[ESSGI CRITICAL GEOHAZARD WARNING] M{mag} Seismic Tremor - {loc}")
      .replace("{mag}", String(Number(eq.magnitude).toFixed(1)))
      .replace("{depth}", String(eq.depth || 10))
      .replace("{loc}", eq.location);
...
app.get("/api/reports/csv/volcanoes", (req, res) => {
  try {
    ...
    res.setHeader("Content-Disposition", `attachment; filename="essgi_volcanoes_catalog_${dateStr}.csv"`);
    res.status(200).send(csvContent);
  } catch (err: any) {
    res.status(500).json({ error: "Failed to generate Volcanoes CSV report", message: err.message });
  }
});
...
app.get("/api/reports/csv/earthquakes", async (req, res) => {
  try {
    ...
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename="essgi_earthquakes_catalog_${dateStr}.csv"`);
    res.status(200).send(csvContent);
  } catch (err: any) {
    res.status(500).json({ error: "Failed to generate Earthquakes CSV report", message: err.message });
  }
});
...
app.get("/api/reports/csv/telemetry", (req, res) => {
  try {
    ...
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename="essgi_integrated_geodisaster_telemetry_${dateStr}.csv"`);
    res.status(200).send(csvContent);
  } catch (err: any) {
    res.status(500).json({ error: "Failed to generate Consolidated Telemetry CSV report", message: err.message });
  }
});
...
app.get("/api/reports/csv/dispatches", (req, res) => {
  try {
    ...
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename="essgi_alert_dispatches_history_${dateStr}.csv"`);
    res.status(200).send(csvContent);
  } catch (err: any) {
    res.status(500).json({ error: "Failed to generate Alert Dispatches CSV report", message: err.message });
  }
});
...
app.post("/api/alerts/trigger-earthquake", (req, res) => {
  ...
  const dispatchResult = processEarthquakeAlert(syntheticEvent, "Manual / API Trigger");
...
app.post("/api/sectors", (req, res) => {
  const { id, title, code, directorate, head, location, description, keyFunctions, activeProjects, stationCount, status, badge, targetTab, metrics } = req.body;
  
  if (!title || !directorate) {
    return res.status(400).json({ error: "Missing required sector fields (title, directorate)" });
  }

  const sectors = getSectors();
  const existingIndex = sectors.findIndex((s: any) => s.id === (id || title.toLowerCase().replace(/[^a-z0-9]/g, "-")));
  ...
  const newSector = {
    ...
    keyFunctions: Array.isArray(keyFunctions) ? keyFunctions : ["Telemetry analysis & geodynamic monitoring"],
    activeProjects: Array.isArray(activeProjects) ? activeProjects : ["National Seismic-Geodetic Mapping"],
    stationCount: stationCount || "Active Telemetry Stations",
    status: status || "ONLINE",
    badge: badge || "Telemetry Ingest",
    targetTab: targetTab || "map",
    metrics: Array.isArray(metrics) ? metrics : [{ label: "Status", value: "Operational" }]
  };
  ...
  saveAuditLog({
    ...
    performedByEmail: String(req.headers["x-user-email"] || "admin@essgi.gov.et"),
  ...
  res.json({ success: true, message: `Sector '${title}' saved successfully!`, sector: newSector });
});
...
app.delete("/api/sectors/:id", (req, res) => {
  ...
  res.json({ success: true, message: `Sector '${target.title}' removed successfully` });
});
...
app.get("/api/news", (req, res) => {
  ...
  if (category && typeof category === "string" && category.toLowerCase() !== "all") {
    newsList = newsList.filter((n: any) => n.category.toLowerCase() === category.toLowerCase());
  }

  if (search && typeof search === "string" && search.trim() !== "") {
    const q = search.toLowerCase();
    newsList = newsList.filter((n: any) => 
...
app.post("/api/news", (req, res) => {
  const { title, excerpt, fullText, category, tag, author, location, keyFindings, recommendations, readTime } = req.body;
  
  if (!title || !excerpt || !fullText) {
    return res.status(400).json({ error: "Missing required news bulletin fields (title, excerpt, fullText)" });
  }
  ...
  const newDispatch = {
    ...
    author: author || String(req.headers["x-user-name"] || "ESSGI Communications Directorate"),
    location: location || "Addis Ababa Headquarters",
    keyFindings: Array.isArray(keyFindings) ? keyFindings : ["Real-time telemetry verification confirmed"],
    recommendations: Array.isArray(recommendations) ? recommendations : ["Maintain continuous geodynamic monitoring"]
  };
  ...
  res.json({ success: true, message: "News dispatch bulletin published successfully!", news: newDispatch });
});
...
app.delete("/api/news/:id", (req, res) => {
  ...
  res.json({ success: true, message: `News bulletin '${target.dispatchCode}' removed successfully` });
});
...
app.post("/api/volcanoes", (req, res) => {
  ...
  const list = getVolcanoes();
  const existingIndex = list.findIndex((v: any) => v.name.toLowerCase() === name.toLowerCase());
...
app.delete("/api/volcanoes/:id", (req, res) => {
  ...
  const volcanoName = volcanoToDelete ? volcanoToDelete.name : "Unknown Volcano";
...
app.get("/api/audit-logs", (req, res) => {
  const userRole = req.headers["x-user-role"];
  if (userRole && userRole !== "admin" && userRole !== "superadmin" && userRole !== "official") {
    return res.status(403).json({ error: "Access denied. Only Admins and Super Admins can retrieve audit logs." });
  }
...
app.get("/api/earthquakes", async (req, res) => {
  try {
    ...
    // Check for high-magnitude events (M >= 5.0) and trigger automated geohazard dispatch alerts
    ...
    console.warn("USGS live feed fetch notice (serving historical dataset):", error?.message || error);
...
app.get("/api/seismic/furi", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to load seismic wave data", message: err.message });
  }
});
...
function generateFallbackReport(summary: any, errorMsg?: string, briefingType: string = "all", customPrompt?: string) {
...
app.post("/api/report", async (req, res) => {
  ...
  if (!process.env.GEMINI_API_KEY || process.env.GEMINI_API_KEY === "MY_GEMINI_API_KEY") {
    ...
    const dynamicReportText = generateFallbackReport(summary, undefined, briefingType, customPrompt);
...
Produce the official briefing report now:`;
    ...
    const { text, modelUsed } = await generateGeminiContentWithFallback(ai, prompt);
    ...
    console.warn("Gemini report generation temporarily unavailable, serving local fallback:", error?.message || error);
    const summary = currentDataSummary || {};
    const dynamicReportText = generateFallbackReport(summary, error?.message || "High Demand", briefingType, customPrompt);
...
app.post("/api/ask-georisk", async (req, res) => {
  ...
  const geologicalKeywords = [
    "volcano", "earthquake", "seismic", "magnitude", "depth", "furi", "erta ale", "dallol", "fentale",
    "alutu", "dabbahu", "dama ali", "kone", "afar", "rift", "danakil", "semara", "semera", "awash",
    "adama", "gnss", "station", "station", "station", "station", "station", "station", "focal",
    "tectonic", "plume", "lava", "sulfur", "so2", "hazard", "risk", "briefing", "drmc", "essgi",
    "tremor", "fault", "graben", "mer", "ears", "escarpment", "alert", "green", "yellow", "orange", "red",
    "infrastructure", "railway", "geothermal", "tsunami", "landslide", "rockfall", "pga", "richter",
    "station", "sensor", "seismograph", "seismogram", "gps", "geodesy", "kibru", "ethiopia", "east africa"
  ];

  const queryLower = userQuery.toLowerCase();
  const isGeologicallyRelevant = geologicalKeywords.some(kw => queryLower.includes(kw));

  // If clearly outside domain (e.g. asking for recipe, movie review, unrelated politics)
  if (!isGeologicallyRelevant && queryLower.length > 15 && !queryLower.includes("data") && !queryLower.includes("system")) {
    return res.json({
      aiGenerated: false,
      grounded: true,
...
Please submit a query related to the current geological dataset or active monitoring channels.`
  ...
  if (!process.env.GEMINI_API_KEY || process.env.GEMINI_API_KEY === "MY_GEMINI_API_KEY") {
    ...
    } else if (queryLower.includes("fentale") || queryLower.includes("awash") || queryLower.includes("alutu")) {
    ...
    } else if (queryLower.includes("earthquake") || queryLower.includes("magnitude") || queryLower.includes("seismic")) {
    ...
    } else if (queryLower.includes("gnss") || queryLower.includes("furi") || queryLower.includes("deformation")) {
...
Draft an absolute maximum of THREE concise sentences summarizing the current geological situation in Ethiopia. Focus strictly on critical active magmatic alerts (like Erta Ale or Dallol), the peak seismicity magnitude, and key immediate recommendations. Keep it direct, clinical, and authoritative for decision-makers. Do not use any markdown formatting or lists.`;
    ...
    console.warn("Gemini summary temporarily unavailable, using local synthesis:", error?.message || error);
    const fallbackText = `Currently in Ethiopia, active tectonic strain persists primarily across the Afar Depression, where 2 volcanoes (Erta Ale and Dallol) remain under elevated critical monitoring due to active magmatic plumes. Seismometers have recorded ${currentDataSummary?.totalEarthquakesInPeriod || 12} events over the past 30 days, with a peak magnitude of M ${currentDataSummary?.highestMagnitudeRecorded || 5.8} near Semera. Local emergency coordinators are advised to increase real-time satellite radar sweeps and restrict public excursions near boiling lava channels.`;
    res.json({
      aiGenerated: false,
      summaryText: fallbackText
